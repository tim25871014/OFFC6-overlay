...
<template>
    ...
    <div id="main" class="absolute h-[1080px] w-[1920px] text-white">
        ...
        <div id="content" v-show="lobbyReady" class="pick-reveal h-[600px] w-full flex gap-15">
            <div v-for="side in ['red', 'blue']" :key="side" class="h-auto w-[50%] flex flex-col"
                ...
                <div class="flex mb-10 items-center" :class="side === 'red' ? '' : 'flex-row-reverse'">
                    ...
                    <div :style="{ backgroundImage: `url(${avatar[side]})` }"
                        class="w-[160px] h-[120px] rounded-[10px] bg-cover bg-center"></div>
                </div>
                <div class="space-y-1">
                    <PlayerList :players="side === 'red' ? redPlayers : bluePlayers" variant="intro"
                        :reverse="side === 'red'" />
                </div>
            </div>
        </div>
        <div id="loading" v-show="!lobbyReady" class="h-[600px] w-full flex justify-center">
            <h1 class="mt-20 loading-text font-rog text-[200px] leading-none select-none" data-text="loading...">
