...
<template>
        ...
        <div id="content" class="h-[720px] w-full flex flex-row">
            <div id="bd-group" class="contents">
                <div id="red-bd"
                    ...
                    <img v-for="(card, idx) in bdInfo.redBD" :key="`red-bd-${idx}`" :src="card"
                ...
                <div id="blue-bd"
                    ...
                    <img v-for="(card, idx) in bdInfo.blueBD" :key="`blue-bd-${idx}`" :src="card"
            ...
            <div id="picks-group" class="contents">
                <div v-for="team in [
                ...
                ]" :key="`${team.id}-picks`" :id="`${team.id}-picks`"
                    ...
                    <div :class="['flex flex-col gap-3 h-full w-full pb-4 pt-2', team.innerAlign]">
                        <div v-for="idx in picksInfo.pickCount" :key="`${team.id}-slot-${idx}`"
                            :class="['w-[445px] flex-1 rounded-[14px] relative overflow-hidden', team.bg]">
                            <div v-if="team.picks[idx - 1]" class="absolute inset-0 pick-reveal z-0"
                            ...
                            <div class="relative h-full w-full flex flex-col justify-center z-20"
            ...
            <div id="cards" class="order-3 w-[678px] h-full pt-[27px] flex flex-col">
                <div id="stadium-outer" class="w-full flex justify-center items-center"
                    ...
                    <div id="stadium" class="w-[90%] h-full flex flex-row gap-[5px]">
                        ...
                        ]" :key="`${team.id}-stadium`" :id="`${team.id}-stadium`"
                            class="w-[50%] h-full flex flex-col gap-[5px] justify-center">
                            <div v-for="(stadium, idx) in team.stadiums.slice(0, cardInfo.isTwoStadiums ? 2 : 1)" :key="`${team.id}-stadium-${idx}`">
                                <div v-if="isBanSectionVisible(stadium)"
                                    class="flex bg-[#253220] border-[#d9ead3] border-[1px] rounded-[10px] items-center gap-2 h-[72px] w-full overflow-hidden">
                                    <img v-if="stadium.id !== 'none'" :src="stadium.imageUrl"
                                ...
                                <div v-if="!isBanSectionVisible(stadium)" class="flex flex-row gap-[5px]">
                                    <div v-for="slot in [0, 1]" :key="slot"
                                        ...
                                        <img v-if="thisSlotBanDetail(stadium, slot).bgUrl !== ''"
                                        ...
                                        <div class="relative z-10">
                                            ...
                                            <p class="font-rog text-[13px] text-shadow-md"
                    ...
                    <div
                        ...
                        <div id="card-play"
                            ...
                            <div v-for="(card, idx) in cardInfo.Cards" :key="`card-${idx}`">
                                <div v-if="card.type === 'play'" class="flex flex-none gap-2 h-[72px] overflow-hidden"
                                    :class="card.team == 'red' ? 'flex-row' : 'flex-row-reverse'">
                                    <div id="card-avatar" class="h-full w-[66px] flex"
                                        :class="card.team === 'red' ? 'items-end justify-end' : 'items-end justify-start'">
                                        <img v-if="card.avatarUrl !== 'none'" :src="card.avatarUrl"
                                    ...
                                    <div id="card-info" :class="[
                                        ...
                                        <div id="card-description"
                                            ...
                                            <div class="flex items-baseline">
                                                <span
                                                    class="text-gray-100 text-[20px] leading-tight break-words font-rog">
                                                    {{ card.id }}
                                                </span>
                                            ...
                                            <div class="flex flex-col">
                                                <div class="grid transition-all duration-300 ease-out"
                                                    ...
                                                    <div class="overflow-hidden">
                                                        <span
                                                            class="font-tpsans-bold text-[16px] whitespace-nowrap block mt-[2px]">
                                                            {{ card.name }}
                                                        </span>
                                                    </div>
                                                </div>
                                ...
                                <div v-else class="h-[72px] flex items-center justify-center">
                                    <div class="pick-reveal w-full h-[48px] flex flex-none font-rog text-[18px] items-center justify-center gap-2"
    ...
    <div id="control-panel" class="absolute left-[1950px] flex gap-3 m-4 text-white flex-col">
        ...
        <div id="mappool" class="text-black font-lexend-regular grid grid-flow-col auto-cols-max grid-rows-10 gap-2">
            <div v-for="(identifier, idx) in mappoolButtons" :key="`mappool-${idx}`">
                <button v-if="identifier !== 'TB'" class="text-black rounded px-2 py-1 text-[20px] w-[60px] text-center"
                    :class="identifier.includes('NM') ? 'bg-blue-300' :
                        ...
                        " @pointerdown.prevent="handlePickAction($event, identifier)" @contextmenu.prevent>
                    {{ identifier }}
                </button>
                <button v-if="identifier === 'TB'"
                    class="text-black rounded px-2 py-1 text-[20px] w-[60px] text-center bg-pink-300"
                    @pointerdown.prevent="handleTBAction($event)" @contextmenu.prevent>
                    TB
                </button>
            </div>
        </div>
        <div id="ex-selector" class="text-black font-lexend-regular flex flex-col gap-2 items-start">
            ...
            <div v-if="selectedStage" class="flex flex-col gap-2">
                ...
                <button v-if="selectedBeatmapIndex !== ''" class="bg-amber-300 text-black rounded px-2 py-1 text-[20px]"
                    @click="addExBeatmap">
                    Add EX
                </button>
            </div>
